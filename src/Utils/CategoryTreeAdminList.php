...
class CategoryTreeAdminList extends CategoryTreeAbstract
{
    ...
    public function getCategoryList(array $categories_array)
    {
        ...
        foreach ($categories_array as $value) {
            $url_edit = $this->urlGenerator->generate('app_admin_edit_category', ['id' => $value['id']]);
            $url_delete = $this->urlGenerator->generate('app_admin_delete_category', ['id' => $value['id']]);

            $this->categorylist .= $this->html_2 . $value['name'] . $this->html_3 . $url_edit .$this->html_4 . ' Edit' . $this->html_5 . $url_delete .
